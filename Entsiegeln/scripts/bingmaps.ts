...
var drawingManager: Microsoft.Maps.DrawingManager;
var infobox: Microsoft.Maps.Infobox;
...
$(document).ready(function () {
	...
	$("#SaveButton").hide();
	$("#CancelButton").hide();
	document.querySelector('.custom-file-input').addEventListener('change', function (e: Event) {
		var name = (document.getElementById("customFileInput") as HTMLInputElement).files[0].name;
		var nextSibling = (e.target as any).nextElementSibling;
	...
	var options = {
		...
		success: function (data) {
			...
			for (var i = 0; i <= Object.keys(data).length; i++) {
				favoriteMap.set(parseInt(Object.keys(data)[i]), Object.values(data)[i] as number);
			}
			FillFavoriteLayer();
...
function FillFavoriteLayer() {
	...
	for (var layer of projectLayers) {
		...
		for (var primitive of primitives) {
			if ((primitive instanceof Microsoft.Maps.Pushpin) && (favoriteMap.has((primitive.metadata as Project).id))) {
				var loc = (primitive as Microsoft.Maps.Pushpin).getLocation();
				var pin = new Microsoft.Maps.Pushpin(loc, { icon: getSVG("#svgLeaf", 10 + 5 * favoriteMap.get((primitive.metadata as Project).id)) });
...
function ShowUserInfo(detailsDialog: boolean) {
	if (sessionStorage.Id) {
...
function LoadSVGs() {
	fetch('/Preferences/GetImages', {
		...
		response.json().then((data: Preferences) => {
...
function toggleMassnahme(e, id: number) {
...
function GetProjectsData(onlynew: boolean) {
	var geturl: string;
	if (sessionStorage.Id) {
...
function GetUserRatings(userId: number) {
	var options = {
		url: "/Ratings/" + userId,
		type: "GET",
		dataType: "json",
...
function DrawProject(project: Project) {
	var prim: Microsoft.Maps.IPrimitive;
	//Module loaded in GetMap()
	prim = Microsoft.Maps.WellKnownText.read(project.wktKoordinaten) as Microsoft.Maps.IPrimitive;
	prim.metadata = project;
	addToEntityLayer(prim);
}

function SaveNewProject(primitive: Microsoft.Maps.IPrimitive) {
	var project: Project = primitive.metadata as Project;
...
function PostNewProject(project: Project, primitive: Microsoft.Maps.IPrimitive) {
	var options = {
		url: "/api",
		type: "POST",
		...
		success: function (response) {
			console.log("Success");
			primitive.metadata = response as Project;
...
function PostNewImage(guidName: string) {
	console.log(guidName);
	var bild: Bild = new Bild();
...
function PostNewRating(project: Project, rating: Rating, callback: Function) {
	var options = {
		url: "/Ratings",
		type: "POST",
...
function PutChangedRating(index: number, rating: Rating, callback: Function) {
	var options = {
...
function PutChangedProject(project: Project) {
	var options = {
		url: "/api/" + project.id,
		type: "PUT",
		data: JSON.stringify(project),
...
function InitData(onlynew: boolean) {
...
function GetMap() {
	var Zentrum: Microsoft.Maps.Location;
	var Bereich: Microsoft.Maps.LocationRect;
...
var searchPin: Microsoft.Maps.Pushpin = null;
...
function addToEntityLayer(primitive: Microsoft.Maps.IPrimitive) {
	...
	if (massnahmenId != -1) {
		...
		if (primitive instanceof Microsoft.Maps.Polyline) {
			var loc = (primitive as Microsoft.Maps.Polyline).getLocations()[0];
			...
			(<Microsoft.Maps.Polyline>primitive).setOptions({ strokeThickness: 4, strokeColor: "green" });
			...
			(<Microsoft.Maps.Polyline>primitive).setOptions({ strokeThickness: 2, strokeColor: "green" });
			projectLayers[massnahmenId].add(primitive);
		}
		...
		borderLayers[0].add(InvertPolygon(primitive as Microsoft.Maps.Polygon));
	}
}

function InvertPolygon(border: Microsoft.Maps.Polygon) {
	Microsoft.Maps.loadModule('Microsoft.Maps.SpatialMath', function () {
		var rings: Microsoft.Maps.Location[][] = new Array<Microsoft.Maps.Location[]>(2);
		...
		var tempPoly = Microsoft.Maps.SpatialMath.Geometry.makeValid(new Microsoft.Maps.Polygon(rings)) as Microsoft.Maps.Polygon;
...
function ShowDialog() {
	...
	($('#SpeichernDialog') as any).on('shown.bs.modal', function (e) {
		($('#Massnahme') as any).trigger('focus');
	});
	var primitiveType = 0;
	...
	var loc;
	if (primitiveType == 1) {
		loc = (drawingShape as Microsoft.Maps.Pushpin).getLocation();
	}
	else {
		loc = (drawingShape as any).getLocations()[0];
	}
	var searchRequest = {
		location: loc,
		callback: function (r: Microsoft.Maps.Search.IPlaceResult) {
			$("#ModalMessage").hide();
			...
			($('#SpeichernDialog') as any).modal("show");
		},
...
function CloseDialog(e: MouseEvent) {
	if ((e.target as HTMLButtonElement).value == "save") {
		...
		project.bilder = new Array<Bild>();
		project.bezeichnung = $("#Bezeichnung").val() as string;
		project.beitragender = $("#Beitragender").val() as string;
		project.userId = sessionStorage.Id;
		project.strasse = $("#Strasse").val() as string;
		project.plz = $("#Plz").val() as string;
		if ($("#Massnahme").val() == 1) {
			project.bpf = true;
			project.pzuB = true;
		}
	...
	($('#SpeichernDialog') as any).modal("hide");
	$("#Bezeichnung").val("");
	$("#Massnahme").val("0");
	$("#Beitragender").val("");
	$("#Strasse").val("");
...
function iPrimitiveClicked(e) {
	closeTooltip();
	var project: Project = (e.target.metadata as Project);
...
function OpenDetailsView(primitive: Microsoft.Maps.IPrimitive) {
	ShowUserInfo(true);
	...
	//}
	g_project = (primitive.metadata as Project);
	...
	$("#btnLegende").bind("click", function () {
		($("#modalLegende") as any).modal("show");
	});
	...
	$("#details-dialog-schliessen").bind("click", function () {
		...
		var loc;
		if (primitive instanceof Microsoft.Maps.Pushpin)
			loc = (primitive as Microsoft.Maps.Pushpin).getLocation();
		else
			loc = (primitive as any).getLocations()[0];
		map.setView({
	...
	$("#favoriteButton").bind("click", function (e) {
		...
		else { console.log("Keine Favoriten mehr vorhanden!"); }
	});
...
function ShowProjectImages(images: Bild[]) {
...
function SaveDetails(project: Project): Project {
	project.details = $("#details-dialog-details").val() as string;
...
function getMassnahme(p: Project): string {
	if (p.bpf || p.pzuB) {
...
function getMassnahmenId(p: Project): number {
	if (p.bpf || p.pzuB) {
		return 0;
	}
	else if (p.kub || p.pentsV || p.azuX || p.gwPI) {
...
function getSvgId(p: Project): string {
	if (p.bpf || p.pzuB) {
		return "#svgBaum";
	}
	else if (p.kub || p.pentsV || p.azuX || p.gwPI) {
...
function postError(c: number, s: string) {
