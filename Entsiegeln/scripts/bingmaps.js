...
function GetProjectsData(onlynew) {
    var geturl;
    if (sessionStorage.Id) {
        geturl = "/api?userId=" + sessionStorage.Id + "&onlynew=" + onlynew;
    }
    else {
        geturl = "/api";
    }
...
function GetMap() {
    var Zentrum = new Microsoft.Maps.Location(52.50627213801224, 13.423410675101962);
    map = new Microsoft.Maps.Map('#myMap', {
        credentials: 'AtjRMMexXb8SbkvunWD_M8mXGoHUPgvmoI3PJ7Hcy4n7-I5ZNJ_FJNoPPP3EqKP_',
        customMapStyle: myStyle,
        center: Zentrum,
        zoom: 14,
        maxBounds: new Microsoft.Maps.LocationRect(Zentrum, 0.15, 0.065)
    });
...
function OpenDetailsView(primitive) {
    ...
    $("#likeButton").bind("click", function (e) {
        ...
        if ($("#likeButton").hasClass("setVoted")) {
            ...
            if (projectRating.length == 0) {
                VoteOff();
                PostNewRating(g_project, { id: undefined, userId: Number(sessionStorage.Id), projectId: g_project.id, like: true, favorite: null }, VoteOn);
            }
            else {
                var index = userRatings.indexOf(projectRating[0]);
                VoteOff();
    ...
    $("#favoriteButton").bind("click", function (e) {
        ...
        if ($("#favoriteButton").hasClass("setVoted")) {
            ...
            if (projectRating.length == 0) {
                VoteOff();
                PostNewRating(g_project, { id: undefined, userId: Number(sessionStorage.Id), projectId: g_project.id, like: null, favorite: true }, VoteOn);
            }
            else {
                var index = userRatings.indexOf(projectRating[0]);
